...
def members_at_date(date):
    ...
    return sorted(set(yahoo_symbol(s) for s in x.loc[x["effective_date"] == latest, "symbol"]
                      if yahoo_symbol(s)))
...
for i, date in enumerate(dates[:-1]):
    ...
    w = 1.0/len(selected)
    new = {s:w for s in selected}

    all_s = set(previous) | set(new)
    turnover = sum(abs(new.get(s,0)-previous.get(s,0)) for s in all_s)
    equity *= 1 - turnover*TRADING_COST

    period_ret = 0.0
    for s, weight in new.items():
        try:
            p0, p1 = monthly.loc[date,s], monthly.loc[next_date,s]
            if pd.notna(p0) and pd.notna(p1) and p0 > 0:
                period_ret += weight*(p1/p0-1)
        except Exception:
            pass
    equity *= 1 + period_ret
    rows.append((next_date,equity))
    for s in selected:
        audit.append({"rebalance_date":date,"symbol":s,"momentum_12_2":signals[s][0],"ID":signals[s][1]})
    previous = new
    rebalance_count += 1

if not rows: raise SystemExit("No backtest results generated.")
eq = pd.Series(dict(rows)).sort_index()

def metrics(x, start=None, end=None):
    x=x.copy()
    ...
    years=max((x.index[-1]-x.index[0]).days/365.25,1/12)
    cagr=(x.iloc[-1]/x.iloc[0])**(1/years)-1
    vol=r.std()*np.sqrt(12)
    sharpe=r.mean()*12/vol if vol>0 else np.nan
    dd=(x/x.cummax()-1).min()
    win=(r>0).mean()
    return total,cagr,vol,sharpe,dd,win,len(r)

def show(title,m):
    print("\n"+title); print("-"*len(title))
    if m is None: print("Insufficient data"); return
    total,cagr,vol,sharpe,dd,win,n=m
    print("Total return:",f"{total*100:.2f}%")
    print("CAGR:",f"{cagr*100:.2f}%")
    print("Annual volatility:",f"{vol*100:.2f}%")
    print("Sharpe ratio:",f"{sharpe:.2f}")
    print("Maximum drawdown:",f"{dd*100:.2f}%")
    print("Winning months:",f"{win*100:.2f}%")
    print("Months tested:",n)

print("\nNIFTY 500 HISTORICAL-MEMBERSHIP")
print("12-2 MOMENTUM + INFORMATION DISCRETION")
print("==========================================")
print("First sort: Top 100 by 12-2 momentum")
print("Second sort: Lowest ID")
print("Final portfolio: Top 50")
print("Trading cost:",f"{TRADING_COST*100:.2f}%")
print("Rebalances:",rebalance_count)
print("Skipped months:",skipped)

show("TRAINING (2020-2023)",metrics(eq,pd.Timestamp("2020-03-31"),TRAIN_END))
show("UNSEEN TEST (2024-2026)",metrics(eq,pd.Timestamp("2024-01-31"),completed_month))
show("FULL PERIOD",metrics(eq))

pd.DataFrame(audit).to_csv("id_strategy_selections.csv",index=False)
print("\nSelection audit saved: id_strategy_selections.csv")
